...
double Vout = 3.3 - 2.5;//Output woltage of LED light//
//int LED[2] = {0,0};
int counter = 3900;
...
//analyze the signal//
  ...
  int find_period(int cur, int label){
    ...
    display_heartrate((heartrate_threshold[0] + heartrate_threshold[1]) / 2);
...
//clear the buffer//
  public: void cleardata(){
    ...
    tem_cur[1] = 0; 
    LEDcur = 0;
    heartrate_threshold[0] = heartrate * 0.8;
    heartrate_threshold[1] = heartrate * 1.2;
    max_threshold[0] = 0;
    max_threshold[1] = 0;
    min_threshold[0] = 0;
    min_threshold[1] = 0;
  ...
  //detect PD signal//
   delayMicroseconds(loop_start + PDdelay - micros());
    switch (i % 4){
      ...
      Serial.println(RED_Signal.get_currentdata());
      //Serial.println();
