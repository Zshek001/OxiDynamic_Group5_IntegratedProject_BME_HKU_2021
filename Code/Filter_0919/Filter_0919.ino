...
//analyze the signal//
  public: int signal_analyze(float current){
    ...
    if(LEDcur > 1 && LEDarray[(LEDcur - 1) % N_LEDarray] >= (maximum + minimum) / 2 && LEDarray[LEDcur % N_LEDarray] <= (maximum + minimum) / 2){
      heartrate = find_period(LEDcur, 0);
    }
    ...
    if(state == 1 && current > (maximum + minimum) / 2){ // if there is peak, calculate the period and the maximum//
      heartrate = find_period(LEDcur, 1);
  ...
  int find_period(int cur, int label){
    ...
    if(label == -1){
      if(tem_cur[1] == 0 || tem_cur[0] == 0 || tem_cur[0] >= tem_cur[1] ){
        return heartrate;
      }
    }
    int heartbeat = 2 * 60 * loop_frequency / 4 / (cur - tem_cur[0]) ;

    //Claer the buffer to detect the next period//
    tem_cur[0] = 0;
    tem_cur[1] = 0; 
